...
public class AVLProdutos {
	...
	public void listaMaisBarato(ARVORE p, double preco) {
		if (p != null) {
			listaMaisBarato(p.esq, preco);
			if(p.dado.getPreco()<preco) {
				System.out.print(" " + p.dado);
			}
			listaMaisBarato(p.dir,preco);
			
		}
	}
	public Produto consultaProduto(ARVORE p, int codigo) {
		if(p!=null) {
			
			if(p.dado.getCodigo()==codigo) {
				return p.dado;
			}else if(p.dado.getCodigo()>codigo) {
				return consultaProduto(p.esq, codigo);
			}else if(p.dado.getCodigo()<codigo) {
				return consultaProduto(p.dir, codigo);
			}
		}
		return null;
	}
